...
export default function HomeScreen() {
  ...
  const resetRoutine = (id) =>
    setRoutines((prev) =>
      prev.map((r) => (r.id === id ? { ...r, progress: 0 } : r))
    );
  ...
  const addRoutineManual = () => {
    ...
    setRoutines((prev) => [
      ...prev,
      { id: Date.now().toString(), label, category, goalPerDay, progress: 0 },
    ]);
    setNewRoutine('');
  ...
  const quickAdd = (category) =>
    setExtraLogs((p) => ({
      ...p,
      [category]: Math.max(0, (p[category] || 0) + 1),
    }));
  ...
  const doneByCat = useMemo(() => {
    ...
    routines.forEach((r) => {
      if (r.category === 'walk')
        sum.walk += Math.min(Math.max(1, r.goalPerDay || 1), r.progress || 0);
      if (r.category === 'meal')
        sum.meal += Math.min(Math.max(1, r.goalPerDay || 1), r.progress || 0);
      if (r.category === 'meds')
        sum.meds += Math.min(Math.max(1, r.goalPerDay || 1), r.progress || 0);
    });
    return {
  ...
  const gallery = Array.isArray(dog?.gallery)
    ? dog.gallery
    : dog?.imageUri
    ? [dog.imageUri]
    : [];
          ...
          <View style={styles.hero}>
            <View style={styles.heroRow}>
              ...
              <View style={styles.heroInfo}>
                ...
                <Text style={styles.nameLine}>
                  저는{' '}
                  <Text style={styles.nameBold}>{dog?.name || 'KONG'}</Text>{' '}
                  에요
                </Text>
                <View style={styles.badgeRow}>
                  <Text style={styles.badge}>{ageText}</Text>
                  {dog?.breed ? (
                    <Text style={[styles.badge, styles.badgeDim]}>
                      {dog.breed}
                    </Text>
                  ) : null}
                </View>
            ...
            <View style={styles.statusWrapHero}>
              {['walk', 'meal', 'meds'].map((k) => {
                ...
                const pct = Math.max(
                  0,
                  Math.min(1, s.total ? s.done / s.total : 0)
                );
                return (
                  <View key={k} style={styles.statusItem}>
                    ...
                    <View style={styles.barWrapHero}>
                      <View
                        style={[styles.barFillHero, { width: `${pct * 100}%` }]}
                      />
                    </View>
                  </View>
                );
          ...
          <View style={styles.sheet}>
            ...
            <View style={styles.coreGrid}>
              ...
              <CoreTile
                ...
                label='Care'
                onPress={() =>
                  navigation.navigate('Care', { selectedDog: dog })
                }
              />
              <CoreTile
            ...
            <View style={styles.presetRow}>
              <Chip
                ...
                icon='footsteps-outline'
                onPress={() =>
                  addPreset({ label: '산책', category: 'walk', goalPerDay: 1 })
                }
              />
              <Chip
                label='식사(2회/일)'
                icon='restaurant-outline'
                onPress={() =>
                  addPreset({ label: '식사', category: 'meal', goalPerDay: 2 })
                }
              />
              <Chip
                label='약(1회/일)'
                icon='medkit-outline'
                onPress={() =>
                  addPreset({ label: '약', category: 'meds', goalPerDay: 1 })
                }
              />
            </View>
                ...
                <>
                  ...
                  {showRoutineInput ? (
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => setShowRoutineInput(true)}
                    >
                      <Ionicons
            ...
            <SectionHeader
              ...
              actionLabel={gallery3.length ? '더보기' : undefined}
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate('IdCard', {
              screen: 'Id',
              params: { photo: dog?.imageUri || null },
            })
          }
        >
